...
class LiveRoomInputBoxController:LiveRoomDelegate, LiveRoomInputBoxServiceProtocol {
    ...
    func showInput() {
        ...
        myViewController.view.frame = CGRectMake(0, 100, 100, 50)
        let s = LiveRoomRegister.shared.getService(LiveRoomUIServiceProtocol.self)
        s?.requestUpdateUI(LiveRoomLayoutDescriptor.LiveRoomLayoutTab, myViewController.view)
    }
