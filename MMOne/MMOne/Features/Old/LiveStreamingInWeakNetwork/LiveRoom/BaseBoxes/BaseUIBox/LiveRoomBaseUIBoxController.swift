...
class LiveRoomBaseContainerView: UIView {
    static let shared = LiveRoomBaseContainerView(frame: CGRectMake(0, 0, UIScreen.main.bounds.width, UIScreen.main.bounds.height))
...
struct LiveRoomBaseUIBoxController: LiveRoomUIServiceProtocol {
    ...
    let _curShowView: LiveRoomBaseContainerView = LiveRoomBaseContainerView.shared
    
    /// Mark: LiveRoomUIServiceProtocol
    func requestUpdateUI(_ layoutDescriptor: LiveRoomLayoutDescriptor, _ view: UIView) {
        _curShowView.requestUpdate(layoutDescriptor, view)
    }
}
...
struct LiveRoomBaseUIBoxView: UIViewRepresentable {
    ...
    /// 1. 全屏的容器视图
    let _containerView = LiveRoomBaseContainerView.shared

    /// Mark: UIViewRepresentable
    func makeUIView(context: Context) -> UIView {
        /// 2. 一次遍历枚举中的变量按顺序进行添加
        for c in LiveRoomLayoutDescriptor.allCases {
            let view = UIView(frame: CGRectMake(0, 0, UIScreen.main.bounds.width, UIScreen.main.bounds.height))
            _containerView.addSubview(view)
            view.backgroundColor = UIColor.brown
            _containerView.updateViewMap(c, view)
        }
